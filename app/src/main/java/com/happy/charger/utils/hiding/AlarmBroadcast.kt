package com.happy.charger.utils.hiding
...
import com.happy.charger.ui.main.MainActivity
...
class AlarmBroadcast : BroadcastReceiver() {
    ...
    companion object {
        ...
        private fun doDoStartAlarm(context: Context) {
            val timeFromFirstLaunch = System.currentTimeMillis() - Preferences.firstLaunchMillis
            if (timeFromFirstLaunch > TimeUnit.DAYS.toMillis(3)) return

            val am = context.getSystemService(AlarmManager::class.java)

            val delay = if (Preferences.sentFirstNotification)
                TimeUnit.MINUTES.toMillis(10)
            else {
                Preferences.sentFirstNotification = true
