...
class PremiumActivity : AppCompatActivity(), Mediator.MediatorCallBack {
    ...
    private var chainIndex = PremiumUserSdk.getChainIndex(this)
    private var mediatorIndex = 0
    private var currentMediator = PremiumUserSdk.getNextMediator(mediatorIndex, chainIndex, this)
    ...
    override fun startLoadAd(mediator: Mediator) {
        ...
        if (mediatorIndex < 1) loadHandler.postDelayed({
            if (!isDisplay) {
                Log.d("12345", "do handler")
                onError()
            }
        }, 5000)
    }
    ...
    override fun onError() {
        ...
        mediatorIndex++
        currentMediator = PremiumUserSdk.getNextMediator(mediatorIndex, chainIndex, this)
        currentMediator?.initMediator(this) ?: finishAndRemoveTask()
    }
