...
import com.app.sdk.sdk.mediator.PangleMediator
...
object PremiumUserSdk {
    ...
    fun getNextMediator(index: Int, chain: Int, callBack: Mediator.MediatorCallBack): Mediator? {
        return when (index) {
            0 -> getFirstMediator(chain, callBack)
            1 -> getSecondMediator(chain, callBack)
            else -> null
        }
    }

    fun getChainIndex(context: Context): Int {
        val index: Int = Prefs.getInstance(context).getChainIndex().takeIf { it < 2 } ?: 0
        Prefs.getInstance(context).saveChainIndex(index + 1)
        return index
    }

    private fun getSecondMediator(chain: Int, callBack: Mediator.MediatorCallBack) = when (chain) {
        1 -> ApplovinMediator(callBack)
        else -> PangleMediator(callBack)
    }

    private fun getFirstMediator(chain: Int, callBack: Mediator.MediatorCallBack) = when (chain) {
        1 -> PangleMediator(callBack)
        else -> ApplovinMediator(callBack)
    }
